...
module('Unit | Model | Band', function(hooks) {
  ...
  test('#isGreatBand', function(assert) {
    ...
    assert.ok(pearlJam.isGreatBand, 'A band with more than 2 good songs is a great band');
    ...
    assert.notOk(stiltskin.isGreatBand, 'A band with fewer than 2 good songs is not a great band');
  });
});
