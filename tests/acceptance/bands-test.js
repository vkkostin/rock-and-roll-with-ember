...
module('Acceptance | Bands', function(hooks) {
  ...
  test('Sort songs in various ways', async function(assert) {
    ...
    assert.equal(currentURL(), '/bands/1/songs?sortBy=titleAsc');

    await click('[data-test-rr=sort-by-rating-asc]');
    assert.equal(currentURL(), '/bands/1/songs?sortBy=ratingAsc');
  ...
  test('Search songs', async function(assert) {
    ...
    assert.equal(currentURL(), '/bands/1/songs?searchTerm=no');

    assert.dom('[data-test-rr=song-list-item]').exists({ count: 2 }, 'The songs matching the search term are displayed');
    await click('[data-test-rr=sort-by-title-desc]');

    assert.ok(currentURL().includes('searchTerm=no'));
    assert.ok(currentURL().includes('sortBy=titleDesc'));
